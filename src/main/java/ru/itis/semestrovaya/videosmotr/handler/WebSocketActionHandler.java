...
import ru.itis.semestrovaya.videosmotr.dto.*;
...
@Component
@EnableWebSocket
public class WebSocketActionHandler extends TextWebSocketHandler {
    private static final Map<String, Room> rooms = new ConcurrentHashMap<>();
    private static final Map<String, Client> clients = new ConcurrentHashMap<>();
    private ObjectMapper objectMapper;

    @Autowired
    public WebSocketActionHandler(ObjectMapper objectMapper) {
    ...
    public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) throws Exception {
        ...
        if (header.equals("message")) {
            ...
            messageEvent.getPayload().setName(clients.get(session.getId()).getUserDto().getEmail());
            ...
            if (!clients.contains(WebSocketActionHandler.clients.get(session.getId()))) {
                clients.add(WebSocketActionHandler.clients.get(session.getId()));
            }
        }
    }
